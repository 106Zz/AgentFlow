...
public class ComplianceSkills {
    ...
    public DeviationInput extractDeviationParams(String userQuery) {
        // 从数据库获取提示词
        String extractPrompt = PromptContextHolder.getSkill("SKILL-EXTRACT-DEVIATION-PARAMS");
        if (extractPrompt == null || extractPrompt.isEmpty()) {
            // 降级：使用硬编码默认提示词
            extractPrompt = """
                ...
                """;
            log.debug("[ComplianceSkills] 使用默认硬编码提示词（数据库未配置 SKILL-EXTRACT-DEVIATION-PARAMS）");
        }
    ...
    public String extractYearContext(String userQuery) {
        // 从数据库获取提示词
        String yearPrompt = PromptContextHolder.getSkill("SKILL-EXTRACT-YEAR");
        if (yearPrompt == null || yearPrompt.isEmpty()) {
            // 降级：使用硬编码默认提示词
            yearPrompt = "你是一个时间意图识别器。请从用户的输入中提取年份信息。" +
                    "如果用户未提及年份，默认返回当前年份 '2026'。" +
                    "仅返回年份数字，不要任何标点符号。";
            log.debug("[ComplianceSkills] 使用默认硬编码提示词（数据库未配置 SKILL-EXTRACT-YEAR）");
        }
